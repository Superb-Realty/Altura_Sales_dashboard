...
function FloorTable({ title, rows }: { title: string; rows: string[][] }) {
  ...
  return <section className="floor-table-section"><h2 className="section-title">{title}</h2>{conditionalColumns.size > 0 && <p className="table-insight conditional-format-legend">Conditional formatting: red 0–40 · yellow 41–60 · green 61–100</p>}<div className="floor-table-wrap"><table className="floor-table"><thead><tr>{rows[0].map((cell, index) => <th key={`${cell}-${index}`}>{cell}</th>)}</tr></thead><tbody>{rows.slice(1).map((row, rowIndex) => <tr key={rowIndex} className={row[0]?.toLowerCase() === 'total' ? 'total-row' : ''}>{rows[0].map((_, index) => { const value = row[index] ?? '—'; return <td key={index}>{index === 0 ? <span className="floor-label">{value}</span> : conditionalColumns.has(index) ? <span className={`performance-band ${bandClass(value)}`}>{value}</span> : value}</td> })}</tr>)}</tbody></table></div></section>
}
...
export default function App() {
  ...
  return <main className="dashboard">
    ...
    {page === 'sales' && <div className="sales-report-grid">
    ...
    <section className="charts inventory-section"><article className="inventory-card"><h3>Area-wise Inventory</h3><div className="inventory-content"><div><ResponsiveContainer width="100%" height={210}><PieChart><Pie data={inventory} dataKey="area" nameKey="status" cx="50%" cy="50%" innerRadius={48} outerRadius={78} paddingAngle={2} label={({ percent }) => `${((percent ?? 0) * 100).toFixed(0)}%`} labelLine={false}>{inventory.map((slice, index) => <Cell key={slice.status} fill={inventoryColors[index]} />)}</Pie><Tooltip formatter={(value: unknown, _name: unknown, item: any) => [`${fmt.format(Number(value) || 0)} sq ft · ${fmt.format(item?.payload?.units ?? 0)} units`, 'Area']} /></PieChart></ResponsiveContainer><p className="legend inventory-legend">{inventory.map((slice, index) => <span key={slice.status} style={{ color: inventoryColors[index] }}>● {slice.status}</span>)}</p></div><aside className="inventory-totals"><p>Total Units<strong>{fmt.format(inventory.reduce((sum, slice) => sum + slice.units, 0))}</strong></p><p>Total Area<strong>{fmt.format(inventory.reduce((sum, slice) => sum + slice.area, 0))}<small>sq ft</small></strong></p></aside></div></article></section></div>
