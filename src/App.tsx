...
function VisitsKpis({ data }: { data: VisitsData }) {
  ...
  const periodLabel = (period: string | undefined) => period?.trim().toLowerCase() === 'previous period' ? 'Previous 28 days' : period
...
function CpMeetingKpi({ data }: { data: VisitsData }) {
  ...
  const periodLabel = (period: string) => period.trim().toLowerCase() === 'previous period' ? 'Previous 28 days' : period
  return <section className="visits-kpis cp-meeting-kpi"><article><div className="kpi-primary"><p>CP Meetings (Last 28 Days)</p><strong>{fmt.format(value)}</strong><div className="kpi-comparison"><span className={delta < 0 ? 'down' : 'up'}>{delta < 0 ? '▼' : '▲'} {Math.abs(delta).toFixed(2)}%</span><small>vs {fmt.format(previousValue)} ({previous ? periodLabel(previous.period) : 'Previous 28 days'})</small></div></div><div className="kpi-breakdown">{totals.map((item) => <p key={item.period}><span>{periodLabel(item.period)}</span><strong>{fmt.format(item.meetings)}</strong><small className="kpi-ltd">(LTD)</small></p>)}</div></article></section>
}
...
export default function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('amber-theme') !== 'light')
