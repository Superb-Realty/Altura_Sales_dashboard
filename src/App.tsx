...
export default function App() {
  ...
  return <main className="dashboard">
    <header className="hero"><h1><em>Amber Live</em> Sales Report</h1><p>Performance summary</p></header>
    ...
    {visits ? <div className="visits-page"><h2 className="section-title">Visits Analysis</h2><VisitsKpis data={visits}/><section className="charts visits-grid"><VisitsAreaChart title="Month-on-Month Visits" data={visits.momVisits}/><VisitsAreaChart title="Week-on-Week Visits" data={visits.wowVisits}/></section><h2 className="section-title visits-subtitle">Channel Performance</h2><div className="cp-trends-row"><CpMeetingKpi data={visits}/><section className="charts meetings-grid"><MeetingsLineChart title="Month-on-Month CP Meetings" data={visits.momCpMeetings} showToDate/></section><section className="charts meetings-grid"><MeetingMixLineChart data={visits.momCpMeetingMix}/></section></div><div className="cp-data-grid"><div className="cp-data-column"><ActiveCpStats title="Active CPs LTD" rows={visits.activeCpsLtd}/><FloorTable title="Top 10 CPs - LTD" rows={visits.topCps}/></div><div className="cp-data-column"><ActiveCpStats title="Active CPs — Last 28 Days" rows={visits.activeCps28Days}/><FloorTable title="Top 10 CPs — Last 28 Days" rows={visits.topCps28Days}/></div></div></div> : <section className="empty-state report-section">Visits data is loading. If this remains empty, sign out and sign in again.</section>}
    <div className="vertical-page"><h2 className="section-title">Vertical Split</h2>{verticalSplit.length ? <section className="charts vertical-chart"><article><h3>Sales Contribution</h3><ResponsiveContainer width="100%" height={310}><BarChart data={verticalSplit} margin={{ top: 24, right: 24, left: 10, bottom: 14 }}><CartesianGrid vertical={false}/><XAxis dataKey="metric"/><YAxis domain={[0,100]} ticks={[0,25,50,75,100]} tickFormatter={(value) => `${value}%`} label={{ value: 'Contribution', angle: -90, position: 'insideLeft' }}/><Tooltip formatter={(value: unknown, name: unknown, item: any) => [`${fmt.format(name === 'CP %' ? item.payload.cp : item.payload.direct)} (${Number(value).toFixed(2)}%)`, String(name)]} /><Bar dataKey="cpPercent" name="CP %" stackId="contribution" barSize={111} fill="#43297C" radius={[0,0,0,0]} isAnimationActive={false}><LabelList dataKey="cpLabel" position="center" className="split-label cp-split-label"/></Bar><Bar dataKey="directPercent" name="Direct %" stackId="contribution" barSize={111} fill="#C8B27F" radius={[5,5,0,0]} isAnimationActive={false}><LabelList dataKey="directLabel" position="center" className="split-label direct-split-label"/></Bar></BarChart></ResponsiveContainer><p className="legend split-legend"><span>● CP %</span><span>● Direct %</span></p></article></section> : <section className="empty-state">Vertical Split data is loading.</section>}</div>
