...
async function handleFloorsRequest(request: Request) {
  ...
  try {
    ...
    const [statement, summary] = await Promise.all(['Floor_Statement!A1:I100', 'Floor_Summary!A1:I100'].map((range) => sheets.spreadsheets.values.get({ spreadsheetId: sheetId, range })))
    return Response.json({ statement: statement.data.values ?? [], summary: summary.data.values ?? [] }, { headers: { 'Cache-Control': 'private, no-store' } })
  } catch (error) { console.error('Floor data request failed', error); return Response.json({ error: 'Unable to load floor data.' }, { status: 502 }) }
}
export default { fetch: handleFloorsRequest }
